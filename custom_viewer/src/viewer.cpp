...
private:
  ...
  // Global index for stream frames.
  bool record_on;
  size_t record_frame;
  size_t record_index;
...
private:
  ...
  void imageViewer()
  {
    ...
    for(; running && ros::ok();)
    {
      ...
      if(mode == IMAGE && record_on)
      {
        saveImages(color, depth);
      }
  ...
  void keyboardEvent(const pcl::visualization::KeyboardEvent &event, void *)
  {
    if(event.keyUp())
    {
      ...
      case 'p':
        record_on = !record_on;
        std::cout << "toggle record: " << (int) record_on << std::endl;
        if(record_on)
          createDirectory();
        break;
      }
  ...
  void createDirectory()
  {
    ...
    record_frame = 0;
    oss.str("");
    oss << "./" << std::setfill('0') << std::setw(4) << record_index;
    record_dir = oss.str();
    struct stat stbuf = {0};
    if(stat(record_dir.c_str(), &stbuf) == -1){
      int dir_ret = mkdir(record_dir.c_str(), 0700);
      if(!dir_ret)
      {
        std::cerr << "Error creating " << record_dir << "!" << std::endl;
        exit(1);
      }
    }
  ...
  void saveCameraMatrices()  
  {
    oss.str("");
    oss<< "./" << record_dir << "/";
    const std::string cameraName = oss.str() + "camera_matrix.yml";
    std::cout << "save camera matrix: " << cameraName << std::endl;
    cv::FileStorage storage(cameraName, cv::FileStorage::WRITE);
    storage << "camera_matrix_color" << cameraMatrixColor;
    storage << "camera_matrix_depth" << cameraMatrixDepth;
    storage.release();
  }

  void saveImages(const cv::Mat& color, const cv::Mat& depth) 
  {
    oss.str("");
    oss << "./" << record_dir << "/" << std::setfill('0') << std::setw(4) << record_frame;
    const std::string colorName = oss.str() + "_color.jpg";
  ...
  void saveCloudAndImages(const pcl::PointCloud<pcl::PointXYZRGBA>::ConstPtr cloud, const cv::Mat &color, const cv::Mat &depth, const cv::Mat &depthColored)
  {
    ...
    const std::string cameraName = baseName + "_camera_matrix.yml";
